...
def mk_station_conv(fn, shinkansen=False):
    '''
    [return]
        {cve駅名: ジョルダン駅名}
    '''
    ...
    if shinkansen:
        ptn = re.compile(r'^(.+)（.+）$')
        return {x[2]: ptn.search(x[3]).group(1) if ptn.search(x[3]) else x[3] for x in data}       
    else:
    ...
    for fn, tt_line, tt_direc in zip(rt_fns, rt_ttlines, rt_ttdirecs):
        with open(os.path.join(rt_dirn, fn), 'r', encoding='utf-8') as f:
            ...
            for i, l in enumerate(f):
                ...
                vehicles.append(Vehicle(uni_key, l))

    print('vehicle count: {}'.format(len(vehicles)))

    return vehicles

def vehicles_match_line(vehicles, lines, reg_stations):

    for i, v in enumerate(vehicles):

        #if (i+1) % 100 == 0:
        #print('{0} / {1} : {2}'.format(i+1, len(vehicles), ctime()))

        # 結果あり -> そのまま
        if v.final_split_runtables:
            continue

        # 対象外駅チェック
        v.chk_stations(reg_stations)

        # 上記チェックOKなら路線マッチング
        if v.result == 0:
            v.match(lines)
            # 複数splitあったら一番路線数すくないsplitにしぼる
            if v.result == 1:
                try:
                    v.select_min_split()
                except:
                    print(v.uni_key, len(v.possible_split_runtables))
    return vehicles

def vehicles_chklist(vehicles, okfn, posfn, errfn):
    ...
    with open(okfn, 'w', encoding='utf-8') as okf, \
        ...
        posf.write('\t'.join(header + ['候補no', 'split no', '路線ID', '路線名', '->runtable']) + '\n')
        ...
        for i, v in enumerate(vehicles):
            ...
            if v.result < 0:
                outline = outhead + [str(v.result)]
                if v.mid_na_idx:
                    outline += ['mid na->'] + [v.stations[i] for i in v.mid_na_idx] 
                if v.outside_na_idx:
            ...
            if v.result == 0:
                for i2, split in enumerate(v.final_split_runtables):
                    outline = outhead + [str(i2), split.lineID, split.linename] \
                            + [y for x in split.runtable for y in x]
                    okf.write('\t'.join(outline) + '\n')
                if v.outside_na_idx:
                    outline = outhead + ['-1', 'outside_na'] \
                            + [x for i3 in v.outside_na_idx for x in [str(i3), v.stations[i3]]]
                    okf.write('\t'.join(outline) + '\n')
                continue

            if v.result == 1:
                for j, pos in enumerate(v.possible_split_runtables):
                    outhead_plus_kohono = outhead + [str(j)]
                    for j2, split in enumerate(pos):
                        outline = outhead_plus_kohono + [str(j2), split.lineID, split.linename] \
                                + [y for x in split.runtable for y in x]
                        posf.write('\t'.join(outline) + '\n')
                if v.outside_na_idx:
                    outline = outhead + ['-1', 'outside_na'] \
                            + [x for j3 in v.outside_na_idx for x in [str(j3), v.stations[j3]]]
                    posf.write('\t'.join(outline) + '\n')
