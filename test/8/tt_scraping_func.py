...
def get_runtable_zairaisen(soup, heading2_h2):
    ...
    # 運行表の「on駅名」（入力駅名と違うことがある）
    rt_on_station = tbl.find('tr',{'class': 'on'}).find('td', {'class': 'eki'}).text
    runtable = []
    for tr in tr_list:
    ...
    return rt_on_station, rt_line, rt_dest, rt_direc, runtable, messages
...
def get_runtable_shinkansen(soup, heading2_h1):
    ...
    if mob1:
        ...
        mob2 = ptn_h_big_h2.search(h_big_h2)
        if mob2:
            rt_direc = mob2.group(1)
        else:
    ...
    return rt_on_station, rt_line, rt_dest, rt_direc, vehicle_no, runtable, messages
...
def get_runtable(href):
    ...
    try:
        ...
        return None, None, None, None, None, [], messages
    else:
        # 在来線、新幹線 分岐
        if heading2_h2:  # 在来線
            rt_on_station, rt_line, rt_dest, rt_direc, runtable, messages \
                = get_runtable_zairaisen(soup, heading2_h2)
            vehicle_no = None
        elif heading2_h1:  # 新幹線
            rt_on_station, rt_line, rt_dest, rt_direc, vehicle_no, runtable, messages \
...
def timetable2runtable(timetable):
    ...
    [return]
        runtableByVehicle:
            [[tt時刻, tt文字列1, tt文字列2, rt_on駅名, rt路線, rt行き先, rt方面, 列車no, runtable], ...]
    ...
    for vehi in timetable:
        ...
        rt_on_station, rt_line, rt_dest, rt_direc, vehicle_no, runtable, messages = get_runtable(rt_href)

        if messages:
            messages_out += [', '.join([y or '' for y in [tt_time, rt_on_station, rt_line, rt_dest, \
                                                          rt_direc, vehicle_no, x]]) \
                             for x in messages]

        data = [tt_time, tt_cha1, tt_cha2, rt_on_station, rt_line, rt_dest, rt_direc, \
                vehicle_no, runtable]
...
def staLineDirec_list2runtable(sta_line_direc_list, dw='1'):
    ...
    [return]
        runtableByVehicle_out:
            [[入力駅, 入力路線, tt方面, tt時刻, tt文字列1, tt文字列2,
              rt_on駅名, rt路線, rt行き先, rt方面, 列車no, runtable], ...]
        runtable (運行表): [[駅, 時刻, 発or着], ...]
        messages_out
    '''
    # 定数（runtableByVehicle のカラム位置）
    _, _, _, _, _, _, i_rt_on_station, _, i_rt_dest, _, i_vehicle_no, i_runtable = range(12)
        ...
        for line, direc_list in line_direc_list:
            ...
            for tt_direc in direc_list:
                ...
                for rt_1vehi in runtableByVehicle:
                    _, _, _, rt_on_station, _, rt_dest, _, vehicle_no, runtable = rt_1vehi
                    ...
                    # 在来線などの場合(列車noがない)
                        ...
                        if [rt_dest, runtable] in zairai_comp:
                            ...
                            rt_on_station_old = runtableByVehicle_out[idx][i_rt_on_station]
