...
if __name__ == '__main__':
    ...
    # 運行表データを取得
    runtableByVehicle, messages = staLineDirec_list2runtable(sta_line_direc_list)
    ...
    # 路線・方面で分ける
    runtableByLineDirec = {}
    for v_data in runtableByVehicle:
    ...
    for k, v in runtableByLineDirec.items():
        ...
        with open(fn, 'w', encoding='utf-8') as f:
            f.write('\t'.join(['入力駅', 'tt時刻', 'tt文字列1', 'tt文字列2', 'rt路線', 'rt行き先', \
                               'rt方面', '列車no', '->runtable']) + '\n')
            for v_d in v:
