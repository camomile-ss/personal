...
def mk_line_station_list(fname='tt_scraping_input.txt'):
    ''' 駅リストさくせい '''
...
def get_time_tables(line, station):
    ...
    if len(tables) == 0:
        ...
        for suf in ['（愛知）', '（名鉄）', '（岐阜）']:
            ...
            if tables:
                #print('{0} {1} OK.'.format(ls[0], ls[1] + suf))
                break
...
if __name__ == '__main__':
    ...
    args = psr.parse_args()
    dirn = args.dirname
    infn = os.path.join(dirn, 'tt_scraping_input.txt')  # 路線名, 駅名 tsv
    outfn = os.path.join(dirn, 'time_table_raw.txt')

    # 駅リスト
    lines_stations = mk_line_station_list(infn)
    ...
    # 運行表hrefつき時刻表データを出力
    print('{0} writing..'.format(outfn))
    with open(outfn, 'w', encoding='utf-8') as ttf:
